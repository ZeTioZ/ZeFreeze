...
public class ZeFreezeCommand implements TabExecutor, FilesManagerUtils.ReloadableFiles {
    ...
    private void handleInfoCommand(CommandSender sender, String[] args) {
        ...
        sendMessage(sender, messages.getStringList("target-freeze-info"), prefix,
                "{freezer}", freeze.getFreezer(),
                "{reason}", freeze.getReason(),
                "{loc_x}", decimalFormat.format(freeze.getLocation().getX()),
                "{loc_y}", decimalFormat.format(freeze.getLocation().getY()),
                "{loc_z}", decimalFormat.format(freeze.getLocation().getZ()));
    }
